...
gallery.addEventListener('click', event => {
  ...
  const image = event.target.closest('a')?.querySelector('img');
  if (!image) return;

  const { source: originalImage, alt: description } = image.dataset;

  const lightbox = basicLightbox.create(
    `<img src="${originalImage}" alt="${description}" width="1112" height="640"> `,
    {
      onShow: () => {
